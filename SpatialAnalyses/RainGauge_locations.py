...
mo_gauges= pd.DataFrame({'Location' : ["00513_bingley-no-2","00515_huddersfield-oakes","00516_bradford", 
                                             "00521_emley-moor", "00523_leeds-weather-centre",
                                             "00529_ryhill","00534_bramham", "56958_emley-moor-no-2" ], 
...
ea_gauges = pd.DataFrame({'ID' : ["063518","079621","080282", "076204", "076413"], 
...
cc_gauges= pd.DataFrame({'Location' : ["Pottery Fields", ], 
                         'Latitude' : [53.784921], 
                         'Longitude' : [-1.541380]})
...
# Add gauge data
for lon, lat in zip(np.array(ea_gauges['Longitude']), np.array(ea_gauges['Latitude'])):
    folium.Marker(location=[lat, lon],  icon=folium.Icon(color='red')).add_to(m)
for lon, lat in zip(np.array(cc_gauges['Longitude']), np.array(cc_gauges['Latitude'])):
    folium.Marker(location=[lat, lon],  icon=folium.Icon(color='green')).add_to(m)
for lon, lat in zip(np.array(mo_gauges['Longitude']), np.array(mo_gauges['Latitude'])):
    folium.Marker(location=[lat, lon],  icon=folium.Icon(color='orange')).add_to(m)
