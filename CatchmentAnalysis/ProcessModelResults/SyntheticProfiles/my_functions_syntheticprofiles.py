...
def create_binned_counts_and_props_hazard_cat_change(fps):
    ...
    for num, df in enumerate([counts_df,proportions_df]):
        df['Cluster_num']= ['Hazard_3CatsLower','Hazard_2CatsLower', 'Hazard_1CatsLower', 'Hazard_SameCat', 'Hazard_1CatsHigher', 'Hazard_2CatsHigher']
        del df['values']
...
def reformat_counts_and_props(cluster_results, column_names, short_ids):
    ...
    # Reformat column names (for the purpose of plotting)
    new_col_names = []
    for column_name in column_names:
        if any((match := substring) in column_name for substring in ['_propcells', '_propcells_urban', '_countcells', '_countcells_urban', '_numcells']):
            new_col_names.append(column_name.replace(match, ''))
    
    counts.columns = new_col_names
