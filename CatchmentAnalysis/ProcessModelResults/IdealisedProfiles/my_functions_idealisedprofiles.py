...
def plot_totals_urban(cluster_results, short_ids, title):
    ...
    for i, v in enumerate(cluster_results['UrbanFloodedArea'].values.tolist()):
        axs[0].text(xlocs[i] - 1.2, v * 1.025, str(cluster_results["%Diff_UrbanFloodedArea_fromSP_formatted"][i]), fontsize = 19, rotation =90)

    ##############################
    # Plot percent difference from single peak
    ##############################
    axs[1].bar(np.arange(len(cluster_results['%Diff_UrbanFloodedArea_fromSP'][1:])), cluster_results['%Diff_UrbanFloodedArea_fromSP'][1:], width = 0.9, color = cluster_results['colour'][1:])
    # Create names on the x-axis
