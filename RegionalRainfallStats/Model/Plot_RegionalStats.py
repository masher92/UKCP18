...
for region in ['leeds', 'leeds-at-centre', 'Northern', 'UK']:
    for overlapping in ['_overlapping', '']:
        ...
        for em in ems:
            ...
            for stat in stats:
                  ...
                  stat_cube = iris.load('Outputs/RegionalRainfallStats/NetCDFs/Model/Allhours/EM_Data/em_{}_{}{}.nc'.format(em, stat, overlapping))[0] 
                  if stat!= 'whprop':
        ...
        for stat in stats:
            ...
            if shared_axis == True:
                ...
                colorbar.ax.tick_params(labelsize=15)
                if stat != 'whprop':
                  colorbar.set_label('mm/hr', size = 25)
                elif stat == 'whprop':
                  colorbar.set_label('%', size = 25)                
                colorbar.ax.set_yticklabels(["{:.{}f}".format(i, n_decimal_places) for i in colorbar.get_ticks()])    
                filename = "Scripts/UKCP18/RegionalRainfallStats/Model/Figs/{}/Allhours/{}{}.png".format(region, stat, overlapping)
            
            # Determine filename for saving file
            elif shared_axis == False:
                filename = "Scripts/UKCP18/RegionalRainfallStats/Model/Figs/{}/Allhours/{}_diffscales{}.png".format(region, stat, overlapping)
