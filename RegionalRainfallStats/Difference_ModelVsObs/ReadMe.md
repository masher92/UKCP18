...
For the 99.5, 99.75th and 99.9th percentiles and the maximum precipitation intensities, the modelled values are higher than the observations across the whole country.(check if some of the values are 0?). This could be because there is a wet bias in the model; however, it may also result from the gridded observations dataset lacking extreme values due to smoothing in the interpolation process. The maximum percentage difference between these values gets higher at higher percentiles, up to a maximum difference of 800% in the maximum values. The model gets the values of extremes too high. The higher bias in the extremes compared to the mean may indicate that the wet bias is due to the observations not capturing extremes due to smoothing, rather than a geniune wet bias in the model.  Looking at the 'leeds-at-centre' JJA max plot, it seems there is some spatial clustering of lower percentage difference values, and looking at the plot of the distance to the rain gauge, it seems that this may correlate with the location of gauges (could check this by overlaying location of rain gauges onto the JJA max plot).  
