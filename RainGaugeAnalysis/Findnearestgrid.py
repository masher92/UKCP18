...
monthly_cubes_list = iris.load(filename,'rainfall_amount')[0]
## Trim concatenated cube to outline of leeds-at-centre geodataframe
obs_cube = trim_to_bbox_of_region_obs(monthly_cubes_list, leeds_at_centre_gdf)
        ...
        if res ==True and res_old != True:
            ...
            for em in ems:
                ...
                if em == '01':
                    ...
                    plot =leeds_at_centre_gdf.plot(ax=ax, categorical=True, alpha=1, edgecolor='black', color='none', linewidth=2)
                    plt.plot(lon_wm, lat_wm,  'o', color='black', markersize = 10)     
                    plt.savefig('Scripts/UKCP18/RainGaugeAnalysis/Figs/CheckingLocations/UKCP18/{}_{}.png'.format(station_name, em),
                                bbox_inches = 'tight')
                    plt.show()
