...
# for file in *.tar ; do
for file in metoffice-c-band-rain-radar_uk_20040407_1km-composite.dat.gz.tar ; do
  echo $file
  tar xf $file 
  gzip -d *.dat.gz
  ssh -A cloud9 "conda activate pygeospatial; python /nfs/a319/gy17m2a/PhD/Scripts/DataDownload/NIMROD/merge_radar_molly.py" $year
#   rm $file
#   rm *.dat
#   rm *dat.gz
done
